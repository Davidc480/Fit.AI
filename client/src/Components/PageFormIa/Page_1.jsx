...
const PageOne = () =>{
    ...
    const handleChange = (e)=>{
        if(validatePage_1(e.target.value).valid){
            setValueInput({enabled: true, value: e.target.value})
            setErrors("")
        } else {
    ...
    return (
        ...
        <div class="text-center font-inter text-2xl xl:text-4xl 2xl:text-6xl not-italic font-bold leading-normal mt-24 xl:mt-12">
                ...
                <div>
                    <form >
                        <div >
                            <input type="text" class="w-full bg-transparent border-b-2 xl:border-b-4 border-b-white focus:ring-0 focus:outline-none text-xl xl:text-2xl 2xl:text-5xl" placeholder="Ingrese su nombre" onChange={handleChange}/>
                        </div>
                    </form>
                    <div>
